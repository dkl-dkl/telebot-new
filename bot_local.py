...
def send_welcome(message):
    ...
    with open('template_text/wel.txt', mode='r', encoding='utf-8') as f:
        content = f.read()
...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = 'Diva K',
            BATCH_ACADEMY = 'Product Team',
            GITHUB_REPO_LINK = 'https://github.com/divakartika'
        )
...
#             [df_plot.iloc[:, i].idxmax() for i in range(df_plot.shape[1])],
#             [df_plot.iloc[:, i].idxmin() for i in range(df_plot.shape[1])]))
